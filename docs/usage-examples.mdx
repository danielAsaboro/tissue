...
| `/decisions` | Decision feed with regime badges, hash-chain verify button, on-chain commitment timeline, TxLINE input verification list |
...
## Simulated execution disclaimer

<Warning>
Live mode publishes risk-approved quotes only. It never invents a fill, a counterparty,
or PnL — there is no sponsor on-chain intent-book to fill against. Simulated fills
exist only in replay/research mode and are always marked `simulated: true`, end to end,
in the ledger, the API, and the dashboard.
</Warning>
