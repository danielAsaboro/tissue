...
To keep this documentation honest: Tissue does not execute real trades. TxLINE's
current on-chain program has no intent-book or order-matching instruction, so live
mode publishes risk-approved quotes and never invents a fill, a counterparty, or PnL.
Simulated fills exist only in replay/research mode and are always labeled `simulated`.
See [Limitations](/faq#limitations) for the full list of what depends on TxLINE
